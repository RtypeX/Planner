                  ...
                  <div
                    ...
                    className={`rounded-lg p-2.5 border flex items-center gap-2 text-xs
                      ${qualifies
                        ...
                        : 'border-[var(--glass-stroke)] bg-[var(--glass-bg-thin)] opacity-70'}`}
                  >
                    <span className="font-mono font-bold tabular-nums text-[var(--label-1)]">{a.afsc}</span>
                    <span className="text-[var(--label-2)] truncate flex-1">{a.name}</span>
                    <span className="text-[10px] text-[var(--label-3)] tabular-nums shrink-0">{reqText}</span>
                    {qualifies && <span className="text-emerald-400 shrink-0">✓</span>}
                  </div>
                )
              })}
            </div>
        ...
        <div className="card-padded">
          ...
          {tests.length === 0 ? (
            ...
            <ul className="space-y-1.5">
              {tests.map((t) => (
                <li key={t.id} className="flex items-center justify-between glass-tile px-3 py-2 text-sm">
                  <div className="min-w-0 flex items-center gap-3">
                    <div className="flex items-center justify-center w-9 h-9 rounded-lg font-extrabold text-sm tabular-nums shrink-0"
                         style={{
                    ...
                    <div className="min-w-0">
                      <div className="text-xs text-[var(--label-3)]">{fmtDateShort(t.date)}</div>
                      {t.lines && (t.lines.G || t.lines.M || t.lines.A || t.lines.E) ? (
                        <div className="text-[10px] font-mono tabular-nums text-[var(--label-2)] mt-0.5">
                          {['M','A','G','E'].filter((k) => t.lines?.[k] != null).map((k) => `${k}${t.lines[k]}`).join(' · ')}
                        </div>
                      ) : null}
                      {t.notes && <div className="text-[11px] text-[var(--label-2)] truncate">{t.notes}</div>}
                    </div>
                  </div>
                  <button className="btn btn-ghost btn-icon hover:!text-rose-500" onClick={() => removeTest(t.id)} aria-label="Delete">
                    <Trash2 size={14} />
                  </button>
                </li>
              ))}
        ...
        <div className="card-padded">
          ...
          {studies.length === 0 ? (
            ...
            <ul className="space-y-1.5 max-h-72 overflow-y-auto pr-1">
              {studies.map((s) => (
                <li key={s.id} className="flex items-center justify-between glass-tile px-3 py-2 text-sm">
                  <div className="min-w-0 flex-1">
                    <div className="font-semibold truncate text-[var(--label-1)]">{s.section}</div>
                    <div className="text-[11px] text-[var(--label-3)]">
                      {fmtDateShort(s.date)} · <span className="tabular-nums">{s.duration} min</span>
                    </div>
                  </div>
                  <button className="btn btn-ghost btn-icon hover:!text-rose-500" onClick={() => removeStudy(s.id)} aria-label="Delete">
                    <Trash2 size={14} />
                  </button>
                </li>
              ))}
  ...
  return (
    <div className="rounded-xl glass-tile p-3">
      <div className="flex items-baseline justify-between">
        <span className="font-mono font-extrabold text-lg text-violet-300">
          {code}
        </span>
        <span className="text-2xl font-extrabold tabular-nums tracking-tight text-[var(--label-1)]">
          {value || '—'}
        </span>
      </div>
      <div className="text-[10px] uppercase tracking-wider font-semibold text-[var(--label-3)] mt-1">
        {label}
      </div>
    </div>
  )
